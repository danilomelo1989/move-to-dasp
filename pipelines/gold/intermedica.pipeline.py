...
def generate_hospitalization_key(table):
    ...
    hospitalization_key_A = table \
        .filter('NUMERO_GUIA_CONTA != "nan"') \
        .filter('''
            DESCRICAO_TIPO_SINISTRO = "Internação"
        ''') \
...
def define_main_procedures(table):
    ...
    main_procedure = table \
        ...
        .withColumn(
            "rank", F.row_number().over(Window.partitionBy("NUMERO_GUIA_CONTA").orderBy(F.desc("VALOR_PAGO")))
        ).filter("rank = 1") \
        .select(
            F.col("NUMERO_GUIA_CONTA"), 
    ...
    #TODO Checar se NUMERO_GUIA_CONTA e chave_internacao_aon dao diferentes resultados
...
table = conn.get_table_from_snowflake('SINISTRO_INTERMEDICA', 'DATABRICKS_SILVER')
...
conn.save_snowflake_table(table, 'INTERMEDICA_CLASSIFICACAO', 'GOLD', 'Overwrite')
