...
from modules.customers.sulamerica.carrier import CarrierSulAmerica
...
def execute_pipeline_copart(file_type: str, date: str):
    context = CarrierSulAmerica()
    run_id = generate_run_id()
    date_time = generate_datetime()
    zip_name, zip_file = context.get_file(file_type, date)
    df = context.handle_files_copart(zip_name, zip_file)
...
def execute_pipeline_managerial(file_type: str, date: str):
    ...
    date_time = generate_datetime()
    zip_name, zip_file = context.get_file(file_type, date)
    df = context.handle_files_managerial(zip_name, zip_file,14)

    for dataframe in df:
        df_standart = standardize_table(context.config, dataframe, file_type)
        context.upload_to_snowflake(df_standart, run_id, date_time)
